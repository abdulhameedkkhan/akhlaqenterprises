...
class SyncCephalopodsCommand extends Command
{
    protected $signature = 'products:sync-cephalopods';

    protected $description = 'Move/update cephalopod products to Cephalopods; add missing with description, no image';
    ...
    {
        $category = Category::where('name', 'Cephalopods')->first();
        if (!$category) {
            $this->error('Category "Cephalopods" not found.');
            return self::FAILURE;
        }
        ...
        foreach ($this->productNames as $desiredName) {
            ...
            if ($existing) {
                ...
                if (!empty($updates)) {
                    ...
                    $this->line("Updated: {$existing->name} → {$desiredName}, category → Cephalopods");
                } else {
                    $unchanged++;
                }
                continue;
            ...
            if ($sameProduct) {
                ...
                $this->line("Same product: \"{$oldName}\" → \"{$desiredName}\", moved to Cephalopods");
                $updatedName++;
                $moved++;
                continue;
            }
        ...
        $this->info("");
        $this->info("Moved/updated to Cephalopods: {$moved}, Name updated: {$updatedName}, Added: {$added}, Unchanged: {$unchanged}");
        return self::SUCCESS;
    }
