...
class SyncLobstersCommand extends Command
{
    protected $signature = 'products:sync-lobsters';

    protected $description = 'Move/update lobster products to Lobsters; add missing with description, no image';
    ...
    public function handle(): int
    {
        ...
        foreach ($this->productNames as $desiredName) {
            ...
            if ($existing) {
                ...
                if (!empty($updates)) {
                    ...
                    $this->line("Updated: {$existing->name} → {$desiredName}, category → Lobsters");
                } else {
                    $unchanged++;
                }
                continue;
            ...
            if ($sameProduct) {
                ...
                $this->line("Same product: \"{$oldName}\" → \"{$desiredName}\", moved to Lobsters");
                $updatedName++;
                $moved++;
                continue;
            }
        ...
        $this->info("");
        $this->info("Moved/updated to Lobsters: {$moved}, Name updated: {$updatedName}, Added: {$added}, Unchanged: {$unchanged}");
        return self::SUCCESS;
    }
