...
class SyncCrabsCommand extends Command
{
    protected $signature = 'products:sync-crabs';

    protected $description = 'Move/update crab products to Crabs; add missing with description, no image';
    ...
    {
        $category = Category::where('name', 'Crabs')->first();
        if (!$category) {
            $this->error('Category "Crabs" not found.');
            return self::FAILURE;
        }
        ...
        foreach ($this->productNames as $desiredName) {
            ...
            if ($existing) {
                ...
                if (!empty($updates)) {
                    ...
                    $this->line("Updated: {$existing->name} → {$desiredName}, category → Crabs");
                } else {
                    $unchanged++;
                }
                continue;
            ...
            if ($sameProduct) {
                ...
                $this->line("Same product: \"{$oldName}\" → \"{$desiredName}\", moved to Crabs");
                $updatedName++;
                $moved++;
                continue;
            }
        ...
        $this->info("");
        $this->info("Moved/updated to Crabs: {$moved}, Name updated: {$updatedName}, Added: {$added}, Unchanged: {$unchanged}");
        return self::SUCCESS;
    }
