            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
                ...
                <div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="mt-8 p-6 bg-green-50 border border-green-100 rounded-2xl flex items-center justify-between group hover:shadow-xl hover:shadow-green-500/10 transition-all duration-500">
                            <div>
                                <h3 class="text-green-800 font-black text-lg mb-1">WhatsApp Support</h3>
                                <p class="text-green-600 text-sm font-semibold">Instant reply for product queries.</p>
                    ...
                    <form action="{{ route('contact.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="first-name" id="first-name" value="{{ old('first-name') }}" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4" placeholder="First Name" required>
                            </div>
                            <div>
                                <label for="last-name" class="block text-sm font-medium text-slate-700 mb-1">Last Name</label>
                                <input type="text" name="last-name" id="last-name" value="{{ old('last-name') }}" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4" placeholder="Last Name" required>
                            </div>
                        </div>
                        
                        <div>
                            <label for="email" class="block text-sm font-medium text-slate-700 mb-1">Email</label>
                            <input type="email" name="email" id="email" value="{{ old('email') }}" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4" placeholder="you@company.com" required>
                        </div>

                        <div>
                            <label for="phone" class="block text-sm font-medium text-slate-700 mb-1">Phone Number</label>
                            <input type="text" name="phone" id="phone" value="{{ old('phone') }}" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4" placeholder="+1 (555) 000-0000">
                        </div>

                        <div>
                            <label for="message" class="block text-sm font-medium text-slate-700 mb-1">Message / Product Inquiry</label>
                            <textarea name="message" id="message" rows="4" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4" placeholder="Tell us what you are looking for..." required>{{ old('message') }}</textarea>
                        </div>
